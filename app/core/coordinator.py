...
class QueryCoordinator:
    async def run_parallel_queries(self, renavam: str, cpf: str, placa: str | None = None, user: str = "system") -> Dict[str, Any]:
        ...
        if placa:
            ...
            # Combine all results (maintain order: Detran, Sefaz, Bradesco-GRT, Bradesco-Multas)
            results = [detran_res, results_init[0], results_init[1], results_init[2]]

        # Process and handle exceptions
        processed_results = []
        sources = ["DETRAN-RJ", "SEFAZ-RJ", "Bradesco-GRT", "Bradesco-Multas"]
