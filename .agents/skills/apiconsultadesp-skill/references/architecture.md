...
*   **`BudgetCoordinator` (`app/core/budget_coordinator.py`)**: Coordena o workflow de cálculo de orçamentos consolidados. Renavam e CPF são
    recebidos por parâmetro (Chassi é opcional):
    1. Consulta em paralelo o **DETRAN-RJ** (Cadastro, Multas Detalhadas, Nada Consta).
    2. Se houver Comunicação de Venda, descobre o CPF do novo proprietário via **SEFAZ-RJ**.
    3. Consulta débitos de GRT no **Bradesco** (se aplicável).
    4. Consulta débitos de GRM (Multas) no **Bradesco** (sempre).
    5. Consulta débitos de IPVA e Dívida Ativa na **SEFAZ-RJ** (se aplicável).
    6. Agrupa todos os débitos, calcula o orçamento total consolidado e salva no cache Supabase.
